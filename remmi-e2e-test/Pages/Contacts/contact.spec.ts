...
test.describe('Contacts side Menu Tests - Remmi E2E', () => {
  ...
  test('Test 42: Verify searching and scrolling loads relevant contacts', async ({ page }) => {
    const login = new LoginActions(page);
    const contact = new ContactActions(page);
...
test('Test 43: Verify selecting a tag in dropdown filters contacts correctly', async ({ page }) => {
  const login = new LoginActions(page);
  const contact = new ContactActions(page);
